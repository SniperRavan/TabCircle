TabCircle — 安装说明 / Setup Guide
================================================================

【中文】

1. 安装
   把 TabCircle.app 拖进左边的「应用程序」文件夹。
...
2. 首次打开会被系统拦下
   ...
   a) 在「应用程序」里【右键】TabCircle →「打开」→ 再点「打开」
      （只需做一次，之后双击即可）
...
3. 授予「辅助功能」权限
   TabCircle 需要在 Chrome 收到之前拦截 ⌃⇥，macOS 只允许有辅助功能
   权限的应用这样做。首次启动会弹窗引导，或手动前往：

     系统设置 → 隐私与安全性 → 辅助功能 → 打开 TabCircle

   授权后需要重新启动 TabCircle（macOS 只在进程启动时读取权限）。

4. 安装 Chrome 扩展（必需）
   TabCircle 由两部分组成，缺一不可。扩展需要手动加载：

     ① 打开 chrome://extensions
     ② 右上角打开「开发者模式」
     ③ 点「加载已解压的扩展程序」
     ④ 选择源码仓库里的 extension 目录
        （从 https://github.com/sniperravan/TabCircle 下载）
...
5. 关于更新后要重新授权
   ...
   TabCircle 会在启动时检测并弹窗引导，跟着点两下即可。
